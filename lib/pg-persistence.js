...
module.exports = class PgPersistence {
  ...
  // Return a copy of a todo with the indicated ID in the indicated todo list.
  ...
  async deleteTodoList(todoListId) {
    const DELETE_TODO_LIST = "DELETE FROM todolists" +
                             " WHERE id = $1 AND username = $2";
  ...
  async existsTodoListTitle(todoListTitle) {
    const FIND_TODOLIST_TITLE = "SELECT title FROM todolists" +
                                " WHERE title = $1 AND username = $2";

    let result = await dbQuery(FIND_TODOLIST_TITLE,
      todoListTitle, this.username );
    return result.rowCount > 0;
  }
